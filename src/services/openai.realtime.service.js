  ...
  try {
    switch (message.type) {
      ...
      case 'input_audio_buffer.appended':
        // This event confirms that audio was successfully appended
        if (conn) {
          if (!conn._appendedCount) conn._appendedCount = 0;
          conn._appendedCount++;
          if (conn._appendedCount <= 5 || conn._appendedCount % 50 === 0) {
            logger.info(`[OpenAI Realtime] Audio append acknowledged #${conn._appendedCount} for ${callId}`);
          }
        }
        break;
  ...
  try {
      ...
      if (conn.audioChunksSent < 5 || conn.audioChunksSent % 100 === 0) {
          ...
          logger.info(`[OpenAI Realtime] Sending audio chunk #${conn.audioChunksSent + 1} for ${callId}:`, {
              base64Length: audioChunkBase64ULaw.length,
...
/**
 * Test audio conversion chain independently
 */
...
/**
 * Test sending a known good audio chunk to OpenAI
 */
async testAudioSend(callId) {
  const conn = this.connections.get(callId);
  if (!conn || !conn.sessionReady) {
    logger.error(`[OpenAI Realtime] Cannot test audio - connection not ready for ${callId}`);
    return false;
  }

  try {
    // Create 100ms of test audio (800 samples at 8kHz)
    const samples = 800;
    const ulawBuffer = Buffer.alloc(samples);
    
    // Fill with valid μ-law encoded silence/comfort noise
    for (let i = 0; i < samples; i++) {
      // μ-law silence is 0xFF, add slight variation for comfort noise
      ulawBuffer[i] = 0xFF + Math.floor(Math.random() * 3) - 1;
    }
    
    const testAudioBase64 = ulawBuffer.toString('base64');
    
    logger.info(`[OpenAI Realtime] Sending test audio chunk to ${callId}: ${samples} samples (${samples/8}ms)`);
    ...
    // Try to commit
    logger.info(`[OpenAI Realtime] Committing test audio for ${callId}`);
    await this.sendJsonMessage(callId, {
      type: 'input_audio_buffer.commit'
    });
    
    logger.info(`[OpenAI Realtime] Test audio sent successfully to ${callId}`);
    return true;
    
  } catch (err) {
    logger.error(`[OpenAI Realtime] Test audio failed for ${callId}: ${err.message}`);
    return false;
  }
}
