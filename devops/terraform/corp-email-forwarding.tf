...
# - Creates SES domain identity + DKIM
# - Creates Route53 records: MX, SPF, DKIM, DMARC (if zone exists)
# - Adds SES receipt rules to existing active rule set
#   'myphonefriend-email-forwarding' to forward to Lambda
# - Stores raw emails in S3 prior to forwarding
############################################################
