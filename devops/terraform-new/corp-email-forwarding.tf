...
# Route53 MX Record for receiving emails
resource "aws_route53_record" "corp_ses_mx" {
  zone_id = data.aws_route53_zone.biancatechnologies.zone_id
  name    = aws_ses_domain_identity.biancatechnologies.domain
  type    = "MX"
  ttl     = 600
...
# Route53 SPF Record
resource "aws_route53_record" "corp_ses_spf" {
  zone_id = data.aws_route53_zone.biancatechnologies.zone_id
  name    = aws_ses_domain_identity.biancatechnologies.domain
  type    = "TXT"
  ttl     = 600
...
# Route53 DMARC Record (optional but recommended)
