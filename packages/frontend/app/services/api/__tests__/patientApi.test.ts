...
describe("patientApi", () => {
  ...
  it("should get all patients", async () => {
    ...
    const result = await patientApi.endpoints.getAllPatients.initiate(queryParams)(
      store.dispatch,
      store.getState,
      {},
    )
  ...
  it("should update a patient", async () => {
    ...
    const result = await patientApi.endpoints.updatePatient.initiate({
      id: patientId,
  ...
  it("should delete a patient", async () => {
    const result = await patientApi.endpoints.deletePatient.initiate({ id: patientId })(
      store.dispatch,
      store.getState,
      {},
    )
  ...
  it("should assign a caregiver to a patient", async () => {
    const result = await patientApi.endpoints.assignCaregiver.initiate({ patientId, caregiverId })(
      store.dispatch,
      store.getState,
      {},
    )
  ...
  it("should remove a caregiver from a patient", async () => {
    const result = await patientApi.endpoints.unassignCaregiver.initiate({
      patientId,
      caregiverId,
    })(store.dispatch, store.getState, {})
  ...
  it("should get caregivers of a patient", async () => {
    const result = await patientApi.endpoints.getCaregivers.initiate({ patientId })(
      store.dispatch,
      store.getState,
      {},
    )
    ...
    if ("error" in result) {
      throw new Error(
        `Get caregivers of a patient failed with error: ${JSON.stringify(result.error)}`,
      )
    } else {
      expect(result.data).toBeInstanceOf(Array)
    }
