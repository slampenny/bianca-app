...
describe("orgApi", () => {
  ...
  it("should update an organization", async () => {
    const updatedOrg = {
      orgId: org.id as string,
      org: { name: "Updated Org", phone: "0987654321" },
    }
    const result = await orgApi.endpoints.updateOrg.initiate(updatedOrg)(
      store.dispatch,
      store.getState,
      {},
    )
  ...
  it("should fetch organization details", async () => {
    const result = await orgApi.endpoints.getOrg.initiate({ orgId })(
      store.dispatch,
      store.getState,
      {},
    )
    ...
    if ("error" in result) {
      throw new Error(
        `Fetching organization details failed with error: ${JSON.stringify(result.error)}`,
      )
    } else {
      expect(result).toMatchObject({
        data: {
  ...
  it("should delete an organization", async () => {
    let result
    if (org.id) {
      result = await orgApi.endpoints.deleteOrg.initiate({ orgId: org.id })(
        store.dispatch,
        store.getState,
        {},
      )
