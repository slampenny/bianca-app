import { useId, useState, type ChangeEvent } from "react"
import type { TextFieldProps } from "./TextField"

function IconEye() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden>
      <path
        stroke="currentColor"
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"
      />
      <path
        stroke="currentColor"
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"
      />
    </svg>
  )
}

function IconEyeOff() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden>
      <path
        stroke="currentColor"
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.894 7.894L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.242 4.242L9.88 9.88"
      />
    </svg>
  )
}

export type PasswordFieldProps = Omit<TextFieldProps, "type"> & {
  value: string
  onChange: (e: ChangeEvent<HTMLInputElement>) => void
  showPasswordLabel?: string
  hidePasswordLabel?: string
}

export function PasswordField({
  value,
  onChange,
  autoComplete = "current-password",
  inputClassName,
  id: idProp,
  disabled,
  inputTestId,
  showPasswordLabel = "Show password",
  hidePasswordLabel = "Hide password",
  label,
  error,
  helper,
  className,
  required,
  "aria-describedby": ariaDescribedBy,
  ...rest
}: PasswordFieldProps) {
  const reactId = useId()
  const id = idProp ?? reactId
  const [visible, setVisible] = useState(false)
  const errorId = error ? `${id}-error` : undefined
  const helperId = helper ? `${id}-helper` : undefined
  const describedBy = [ariaDescribedBy, errorId, helperId].filter(Boolean).join(" ") || undefined

  return (
    <div className={["bianca-field", "bianca-password-field", className].filter(Boolean).join(" ")}>
      {label != null ? (
        <label className="bianca-field__label" htmlFor={id}>
          {label}
          {required ? (
            <>
              {" "}
              <span aria-hidden="true">*</span>
            </>
          ) : null}
        </label>
      ) : null}
      <input
        id={id}
        type={visible ? "text" : "password"}
        autoComplete={autoComplete}
        className={["bianca-field__input", "bianca-focus-ring", inputClassName].filter(Boolean).join(" ")}
        value={value}
        onChange={onChange}
        disabled={disabled}
        required={required}
        aria-invalid={error ? true : undefined}
        aria-describedby={describedBy}
        data-testid={inputTestId}
        {...rest}
      />
      <button
        type="button"
        className="bianca-password-toggle bianca-focus-ring"
        onClick={() => setVisible((v) => !v)}
        aria-label={visible ? hidePasswordLabel : showPasswordLabel}
        aria-pressed={visible}
        disabled={disabled}
      >
        {visible ? <IconEyeOff /> : <IconEye />}
      </button>
      {error ? (
        <p id={errorId} className="bianca-field__error" role="alert">
          {error}
        </p>
      ) : null}
      {helper && !error ? (
        <p id={helperId} className="bianca-field__helper">
          {helper}
        </p>
      ) : null}
    </div>
  )
}
