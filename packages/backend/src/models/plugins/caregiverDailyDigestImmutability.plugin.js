/**
 * Immutability guards for CaregiverDailyDigest.
 *
 * Sent digests are immutable. All application writes must go through
 * caregiverDailyDigest.service helpers (updateDraftDigest, createDigestVersion, markDigestSent).
 *
 * Guards:
 ...
 *
 * Intentional bypass:
 * - caregiverDailyDigestCleanup.service (compliance redaction via native db.collection)
 * - migrate-mongo scripts using db.collection('caregiverdailydigests') directly
 */
...
const wrapCollectionMutators = (model) => {
  ...
  if (!collection || collection.__digestImmutabilityWrapped) {
    return;
  }
  ...
  collection.__digestImmutabilityWrapped = true;
};

const caregiverDailyDigestImmutabilityPlugin = (schema) => {
