...
resource "aws_s3_bucket_lifecycle_configuration" "hipaa_backups" {
  ...
  # Safety backups before restore: 30 days
  rule {
    id     = "safety-backups-lifecycle"
    status = "Enabled"
...
resource "aws_iam_role_policy" "backup_lambda_policy" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        # SSM Run Command on target EC2 instance(s)
        Effect = "Allow"
        Action = [
          "ssm:SendCommand"
        ]
...
resource "aws_lambda_function" "mongodb_backup" {
  filename         = "${path.root}/lambda-backup.zip"
  function_name    = "${var.environment}-mongodb-backup"
  role             = aws_iam_role.backup_lambda_role.arn
  handler          = "index.handler"
  source_code_hash = fileexists("${path.root}/lambda-backup.zip") ? filebase64sha256("${path.root}/lambda-backup.zip") : null
  runtime          = "nodejs18.x"
  ...
  environment {
    variables = {
      ...
      ENVIRONMENT             = var.environment
      EC2_TARGET_TAG_NAME     = var.ec2_target_tag_name
...
resource "aws_lambda_function" "backup_verification" {
  filename         = "${path.root}/lambda-verify-backup.zip"
  function_name    = "${var.environment}-backup-verification"
  role             = aws_iam_role.backup_lambda_role.arn
  handler          = "verify.handler"
  source_code_hash = fileexists("${path.root}/lambda-verify-backup.zip") ? filebase64sha256("${path.root}/lambda-verify-backup.zip") : null
  runtime          = "nodejs18.x"
  timeout          = 900
...
resource "aws_lambda_function" "mongodb_restore" {
  filename         = "${path.root}/lambda-restore.zip"
  function_name    = "${var.environment}-mongodb-restore"
  role             = aws_iam_role.backup_lambda_role.arn
  handler          = "restore.handler"
  source_code_hash = fileexists("${path.root}/lambda-restore.zip") ? filebase64sha256("${path.root}/lambda-restore.zip") : null
  runtime          = "nodejs18.x"
  timeout          = 900
  memory_size      = 2048
