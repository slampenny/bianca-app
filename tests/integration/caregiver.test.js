...
const { caregiverOne, caregiverTwo, admin, insertCaregivers, insertCaregiverAndReturnToken, insertCaregiverAndReturnTokenByRole } = require('../fixtures/caregiver.fixture');
  ...
  describe('POST /v1/caregivers', () => {
    ...
    describe('GET /v1/caregivers/:caregiverId', () => {
      ...
      test('should return 404 error if caregiver is not found', async () => {
        ...
        await request(app)
          .get(`/v1/caregivers/${caregiverOne.id}`)
          .set('Authorization', `Bearer ${adminAccessToken}`)
          .send()
    ...
    describe('DELETE /v1/caregivers/:caregiverId', () => {
      test('should return 204 if data is ok', async () => {
        ...
        await request(app)
          .delete(`/v1/caregivers/${caregiverOne.id}`)
          .set('Authorization', `Bearer ${caregiverOneAccessToken}`)
          .send()
          .expect(httpStatus.NO_CONTENT);

        const dbCaregiver = await Caregiver.findById(caregiverOne.id);
      ...
      test('should return 401 error if access token is missing', async () => {
        ...
        await request(app).delete(`/v1/caregivers/${caregiverOne.id}`).send().expect(httpStatus.UNAUTHORIZED);
      });
      ...
      test('should return 403 error if caregiver is trying to delete another caregiver', async () => {
        ...
        await request(app)
          .delete(`/v1/caregivers/${caregiverTwo.id}`)
          .set('Authorization', `Bearer ${caregiverOneAccessToken}`)
          .send()
          .expect(httpStatus.FORBIDDEN);
      });
    ...
    describe('PATCH /v1/caregivers/:caregiverId', () => {
      test('should return 200 and successfully update caregiver if data is ok', async () => {
        ...
        const res = await request(app)
          .patch(`/v1/caregivers/${caregiverOne.id}`)
          .set('Authorization', `Bearer ${caregiverOneAccessToken}`)
          .send(updateBody)
          .expect(httpStatus.OK);
      ...
      test('should return 401 error if access token is missing', async () => {
        ...
        await request(app).patch(`/v1/caregivers/${caregiverOne.id}`).send(updateBody).expect(httpStatus.UNAUTHORIZED);
      });
      ...
      test('should return 403 if caregiver is updating another caregiver', async () => {
        ...
        await request(app)
          .patch(`/v1/caregivers/${caregiverTwo.id}`)
          .set('Authorization', `Bearer ${caregiverOneAccessToken}`)
          .send(updateBody)
          .expect(httpStatus.FORBIDDEN);
      });
