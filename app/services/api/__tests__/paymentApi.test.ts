...
describe('paymentApi', () => {
  ...
  beforeEach(async () => {
    ...
    const patientResponse = await createPatientInOrg(org, testCaregiver.email, testCaregiver.password);
    ...
    const convResult = await conversationApi.endpoints.createConversation
      .initiate({ patientId, data: conversationPayload })(store.dispatch, store.getState, {});
  ...
  it('should get all invoices for a patient', async () => {
    ...
    await paymentApi.endpoints.createInvoiceFromConversations
      .initiate({ patientId, payload: invoicePayload })(store.dispatch, store.getState, {});
    
    const queryParams = { patientId, status: 'pending' };
    const result = await paymentApi.endpoints.getInvoicesByPatient
      .initiate(queryParams)(store.dispatch, store.getState, {});
  ...
  it('should get all invoices for an organization', async () => {
    ...
    const result = await paymentApi.endpoints.getInvoicesByOrg
      .initiate(queryParams)(store.dispatch, store.getState, {});
